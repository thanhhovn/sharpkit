    ...
    class CodeModelExporter
    {
        ...
        public void Export(List<Class> list)
        {
            ...
            else
            {
                ...
                {
                    var tokens = ce.Namespace.Replace(RootNamespace, "").Split('.').ToList();
        ...
        public void Export(Class ce)
        {
            ...
            Write("public {0}{1} partial {2} {3}", ce.IsNew.If("new "), ce.IsStatic.If("static "), ce.IsInterface.If("interface", "class"), Identifier(ce.Name));
        ...
        void ExportXmlDoc2(Element me)
        {
            if (ExportXmlDoc)
            {
                ...
                Method met = me as Method;
                if (met != null)
                {
                    foreach (var p in met.Parameters)
                    {
                        WriteLine("/// <param name=\"" + p.Name + "\">" + p.Summary + "</param>");
                    }
                }
